			...
			if(filesForNode != null)
			{
				for(DataObject obj : filesForNode)
				{
					...
					if(hashvalue < weightFactor)
					{
						...
						Commons.messageSender.sendMessage("", Constants.ADD_FILE,Commons.GeneratePayload(obj.fileName,obj.placementGroup, obj.replicaId));
						// Now delete from Source Data Node
						Commons.messageSender.sendMessage("", Constants.DELETE_FILE,Commons.GeneratePayload(obj.fileName,obj.placementGroup, obj.replicaId));
					}
				}
